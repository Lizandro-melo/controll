          ...
          <div className="grow basis-[100%] pt-10 pl-5 pr-5 pb-5 text-center rounded-sm border relative font-semibold flex items-center justify-end">
            ...
            <div className="flex gap-3 text-xs justify-between w-full items-center">
              <div className="flex flex-col gap-2 items-start">
                <span>CPF: {veiculo?.cliente?.num_cpf}</span>
                <span>
                  DATA DE CONTRATO:{" "}
                  {moment(veiculo?.cliente?.data_contrato).toString()}
                </span>
                <span>
                  DATA DE VENCIMENTO:{" "}
                  {moment(veiculo?.cliente?.data_fim_contrato).toString()}
                </span>
                <span>
                  TOTAL PAGO:{" "}
                  {Intl.NumberFormat("pt-br", {
                    ...
                    veiculo?.veiculo?.valor_aluguel! *
                      moment(veiculo?.cliente?.data_contrato).diff(
...
function EditarVeiculo({ veiculo_info, ...props }: Props) {
  ...
  //   queryKey: ["clientes_disponiveis"],
  //   queryFn: async () => {
  //     const res = await axios.get("/api/cliente/list", { headers });
